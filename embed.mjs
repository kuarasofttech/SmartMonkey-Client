...
export const TOOLS = [
  ...
  { name: 'ask_user', description: 'Ask the project owner a question in the app and wait for the answer. The owner answers by CLICKING, so always give 2–6 short options — drawn from what you found in the repo when the question is about this project (multi: true if several can apply). They can still type their own. One question per call.', input_schema: { type: 'object', properties: { question: { type: 'string' }, options: { type: 'array', items: { type: 'string' }, minItems: 2 }, multi: { type: 'boolean' } }, required: ['question', 'options'] } },
...
export function makeToolRunner(cwd, ask, requestConnections = async () => 'No connect step in this context; proceed to build the blueprint.') {
  ...
  return async (name, input = {}) => {
    ...
    if (name === 'ask_user') {
      const options = Array.isArray(input.options) ? input.options.map(String).filter(Boolean) : [];
      if (options.length < 2) return 'Not asked: the owner answers by clicking — call ask_user again with 2–6 short options (drawn from what you found in the repo).';
      return ask(input.question || '', options, input.multi);
    }
