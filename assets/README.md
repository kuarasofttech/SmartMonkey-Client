# SmartMonkey profile kit

This folder teaches SmartMonkey how to test your app. You run one prompt in your
own repo; it writes a `profile.json` describing how a tester gets in, how to
reset state, and what must never happen. **Your source code never leaves your
machine — only `profile.json` does.**

## What's here

| File | What it is |
|------|------------|
| `smartmonkey-profile.md` | The prompt. Run it with your own AI coding agent (Claude Code, Cursor, Codex, Gemini CLI — any frontier model), inside your repo. |
| `view.html` | A local viewer + case editor. Read the profile (what you deliver to us), and view/edit/add test cases and download `cases.json`. Nothing is uploaded. |
| `profile.json` | Produced by the prompt. The one file you upload to SmartMonkey. |
...
2. Open your repo in your AI coding agent and give it the contents of
   `smartmonkey-profile.md`. It **starts by interviewing you** — a few numbered
   questions (where your test cases live, how to reach them, whether to generate
   `cases.json` now) — then reads your code and writes `smartmonkey/profile.json`.
...
3. **See what you're delivering:** open `view.html`.
   - Easiest: run `python3 -m http.server` in this folder and open
     `http://localhost:8000/view.html` — it loads `profile.json` automatically.
   - Or just double-click `view.html` and drag `profile.json` onto it.
   Nothing is uploaded; the page reads the file in your browser only.
4. Upload `profile.json` to SmartMonkey. If you generated `cases.json`, import it there
   too (Test cases → Import JSON → choose file).

## Keeping it fresh

Your app changes; the profile can go stale. It's anchored to the commit it was
built at (the only code reference in the file), and a quick check — run in your
repo — tells you when the code has moved:

```
node smartmonkey-check.mjs profile.json --list
```

- **fresh** — nothing has changed since the profile was built.
...
- **We receive:** `profile.json` — your app's builds, screens, flows, reset
