...
export const ASK_TOOL = {
  name: 'ask_user',
  description: 'Ask the project owner a question in the SmartMonkey app and wait for the answer. Use it for project-specific questions the repo cannot settle and whose answer changes the blueprint. Give 2–6 short `options` whenever the answer is one of a few choices (set `multi: true` if several can apply); the owner can still type their own answer. One question per call.',
  inputSchema: {
    type: 'object',
    properties: {
      question: { type: 'string', description: 'The question, in plain words.' },
      options: { type: 'array', items: { type: 'string' }, description: 'Short choices, when the answer is one of a few.' },
...
export async function handleMessage(msg, ask) {
  ...
  switch (msg.method) {
    ...
    case 'tools/call': {
      if (msg.params?.name !== ASK_TOOL.name) return error(-32602, `unknown tool ${msg.params?.name}`);
      const a = msg.params.arguments || {};
      try {
        const answer = await ask({ question: String(a.question || ''), options: Array.isArray(a.options) ? a.options.map(String) : [], multi: !!a.multi });
...
export function httpAsk(url, token, { fetchImpl = fetch, sleep = ms => new Promise(r => setTimeout(r, ms)) } = {}) {
  ...
  return async ({ question, options, multi }) => {
    const { id } = await call('POST', url, { question, options, multi });
    for (let failures = 0; ;) {
      try {
