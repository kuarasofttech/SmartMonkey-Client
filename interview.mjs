...
export function runBlock(previous = [], connected = []) {
  ...
  return [
    ...
    '',
    '**This run writes the blueprint ONLY** — never `cases.json`. The owner proposes test cases separately (Propose cases, in the app), so don\'t ask what to produce and skip the "Test cases" section below.',
    '',
...
export function rebuildBlock(reviewed = [], connected = []) {
  ...
  return [
    ...
    'This run writes the blueprint ONLY — never `cases.json` (test cases are proposed separately, with Propose cases); ignore any answer above about producing test cases.',
    '',
